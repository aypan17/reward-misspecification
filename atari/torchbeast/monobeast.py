...
parser.add_argument("--fuel_multiplier", default=1.0, type=float,
                    help="Sets the magnitude of the fuel_multiplier proxy (if nonnegative) \
                    or the pacifist proxy (if negative). By default does nothing.")
parser.add_argument("--move_penalty", default=0.0, type=float,
                    help="Sets the magnitude of the move penalty (if nonnegative) \
                    or the deviate_from_center proxy (if negative). By default does nothing")
parser.add_argument("--true_move_penalty", default=0.0, type=float,
                    help="Same as above, except recorded as true_move")
...
def test(flags, num_episodes: int = 4, folder = None, num_layers = None, hidden_size = None):
    ...
    model.eval()
    p = sum(p.numel() for p in model.fc.parameters())
    ...
    logging.info(
        "Average returns over %i episodes: %.1f +/- %.1f", 
        num_episodes, np.mean(returns).item(), np.std(returns).item()
    )
    logging.info(
        "Average true returns over %i episodes: %.1f +/- %.1f", 
        num_episodes, np.mean(true_returns).item(), np.std(true_returns).item()
    )
    logging.info(
        "Average true move over %i episodes: %.1f +/- %.1f", 
        num_episodes, np.mean(true_move).item(), np.std(true_move).item()
    )
    logging.info(
        "Average return/step over %i episodes: %.1f +/- %.1f", 
        num_episodes, np.mean(return_per_step).item(), np.std(return_per_step).item()
    )
    logging.info(
        "Average true return/step over %i episodes: %.1f +/- %.1f", 
        num_episodes, np.mean(true_return_per_step).item(), np.std(true_return_per_step).item()
    )
    logging.info(
        "Average num steps over %i episodes: %.1f +/- %.1f", 
        num_episodes, np.mean(lens).item(), np.std(lens).item()
    )
